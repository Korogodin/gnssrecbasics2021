...
void scale_factor_use(Ephemeris* ep, sub *data); //преобраз эфемерид в необх типы данных
int32_t  convert_eph(string sf, int32_t Begin, int End);//преобраз эфемерид в необх типы данных
...
int main(void)
{
    ...
    if (!file.is_open())
		...
		{
            file >> sec >> coord_from_matlab[0][t] >> coord_from_matlab[1][t] >> coord_from_matlab[2][t];
        }
        file.close();
    }
...
uint32_t splitconvert_eph(string sf, uint16_t Begin, int End, uint16_t Contin, int End_of_Contin)//когда параметр лежит в разных кадрах
{
    uint32_t ans = 0;
    for (int i = Begin; i < End; i++) {
        bool bit = (sf[i - 1] == '1');
...
void scale_factor_use(Ephemeris* ep, sub *data)//учет scale factor-а
{
    ...
    ep->omega = compl2int(splitconvert_eph(data->sf3,197, 205, 211, 235),32)*SF3*Semi_circles*deg2rad;
