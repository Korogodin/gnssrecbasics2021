Контрольная 2: Статистический эквивалент коррелятора
====================================================

# Что здесь?

Это подходящий каталог для того, чтобы хранить промежуточные и итоговые результаты по контрольной. 
...
Допустим, вы написали послание преподавателю в test_correlator_report.doc. 
Положите его сюда. 
...
```sh
git add test_correlator_report.doc test_correlator_report.pdf 
```

Сделайте сам коммит:

```sh
git commit -m "Я молодец, я написал(а) контрольную"
```
...
Это действие называется Pull request - PR - ПР, выполняется через веб-страниц (сверху кнопочка).
...
**Когда вы устраните все замечания, ваш ПР будет принят - это знак того, что КМ выполнено.**
