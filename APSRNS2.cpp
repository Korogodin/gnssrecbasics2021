...
#include <cmath>
#include <time.h>
#include <ctime> // измерение длительности выполнения функции
#include <iostream>
#include <fstream>
#include <windows.h>
...
int main(void)
{
	...
	FILE* fopenfile = fopen("coords.txt", "r");    // считывание координат из матлаба 
	if (fid != nullptr) {
		SF1_3 subframes;
...
void Coordinate_calculation(Ephemeris* ep, uint32_t t, Coord *Position)   // вычисление координат КА
{
	...
	double E_0 = M_k; // трансцедентный алгоритм 
	double E_k = 0;
	int k = 0;
